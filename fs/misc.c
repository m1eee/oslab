...
PUBLIC int do_list() {
	char filename[MAX_PATH];
	// 获取调用者进程的进程号，用于后续进行内存地址转换 (va2la)
	...
	int dir_blk0_nr = dir_inode->i_start_sect; // 获取该目录数据起始的第一个扇区号
	int nr_dir_blks = (dir_inode->i_size + SECTOR_SIZE - 1) / SECTOR_SIZE; // 计算目录所占用的扇区数
	int nr_dir_entries = dir_inode->i_size / DIR_ENTRY_SIZE; // 计算目录中的条目数
	...
	for (int i = 0; i < nr_dir_blks; i++) { 
		...
		for (int j = 0; j < SECTOR_SIZE / DIR_ENTRY_SIZE; j++,pde++) {
			// 将内核缓冲区(fsbuf)中的目录项(pde) 复制到 用户进程的缓冲区(fs_msg.BUF)
			phys_copy((void*)va2la(src, fs_msg.BUF+j*DIR_ENTRY_SIZE),
		  				(void*)pde,	
		  				DIR_ENTRY_SIZE);
			if (++m > nr_dir_entries)
				break;
