...
PUBLIC int do_kill_proc()
{
    int i;
    /* 目标进程ID来自消息体(PID) */
    int pid = mm_msg.PID; 
    
    /* 安全检查：不能杀系统任务 (0 ~ NR_TASKS-1) */
   ...
   /* 通知文件系统 (FS) 释放 inode、关闭文件描述符 */
    ...
    /* 设置退出状态 */
    p->exit_status = 0; // 被 kill 强制杀死，状态通常设为 0 或特定信号值

    /* 处理父进程关系 */
    if (proc_table[parent_pid].p_flags & WAITING) { 
        /* 如果父进程正在 wait()，直接唤醒父进程并清理尸体 */
        proc_table[parent_pid].p_flags &= ~WAITING;
        cleanup(p);
    }
    else { 
        /* 如果父进程没在 wait，当前进程变成"僵尸"(HANGING)，等待父进程收尸 */
        p->p_flags |= HANGING;
    }

    /* 处理子进程 (孤儿进程过继给 INIT) */
    for (i = 0; i < NR_TASKS + NR_PROCS; i++) {
        if (proc_table[i].p_parent == pid) { /* 找到了该进程的子进程 */
            proc_table[i].p_parent = INIT;   /* 认 INIT 为新父进程 */
            
            /* 如果 INIT 正在等待且这个子进程已经是僵尸，让 INIT 立刻回收 */
            if ((proc_table[INIT].p_flags & WAITING) &&
                (proc_table[i].p_flags & HANGING)) {
                proc_table[INIT].p_flags &= ~WAITING;
                cleanup(&proc_table[i]);
